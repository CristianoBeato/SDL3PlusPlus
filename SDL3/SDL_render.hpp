...
class SDLRenderer
{
    /* data */
public:
    SDLRenderer( void ) : renderer( nullptr ){}
    SDLRenderer( const SDLRenderer &ref ) : renderer( ref.renderer ){}
    SDLRenderer( SDL_Renderer* ptr ) : renderer( ptr ) {}
    ~SDLRenderer( void );

    SDL_INLINE bool             Create( const SDLWindow &window, const char *name )
    {
    ...
    SDL_INLINE bool             CreateWindowAndRenderer( const char *title, int width, int height, SDL_WindowFlags window_flags, SDLWindow* &window )
    {
    ...
    SDL_INLINE bool             CreateSoftware( const SDLSurface surface )
    {
    ...
    SDL_INLINE bool RenderTexture( const SDLTexture &texture, const SDL_FRect *srcrect, const SDL_FRect *dstrect)
    {
        return SDL_RenderTexture( renderer, texture, srcrect, dstrect );
    }

    SDL_INLINE bool RenderTextureRotated( const SDLTexture &texture, const SDL_FRect *srcrect, const SDL_FRect *dstrect, double angle, const SDL_FPoint *center, SDL_FlipMode flip )
    {
        return SDL_RenderTextureRotated( renderer, texture, srcrect, dstrect, angle, center, flip );
    }

    SDL_INLINE bool RenderTextureAffine( const SDLTexture &texture, const SDL_FRect *srcrect, const SDL_FPoint *origin, const SDL_FPoint *right, const SDL_FPoint *down)
    {
        return SDL_RenderTextureAffine( renderer, texture, srcrect, origin, right, down );
    }
    
    SDL_INLINE bool RenderTextureTiled( const SDLTexture &texture, const SDL_FRect *srcrect, float scale, const SDL_FRect *dstrect )
    {
        return SDL_RenderTextureTiled( renderer, texture, srcrect, scale, dstrect );
    }

    SDL_INLINE bool RenderTexture9Grid( const SDLTexture &texture, const SDL_FRect *srcrect, float left_width, float right_width, float top_height, float bottom_height, float scale, const SDL_FRect *dstrect )
    {
        return SDL_RenderTexture9Grid( renderer, texture, srcrect, left_width, right_width, top_height, bottom_height, scale, dstrect );
    }

    SDL_INLINE bool RenderGeometry( const SDLTexture texture, const SDL_Vertex *vertices, int num_vertices, const int *indices, int num_indices )
    {
        return SDL_RenderGeometry( renderer, texture, vertices, num_vertices, indices, num_indices );
    }

    SDL_INLINE bool RenderGeometryRaw( const SDLTexture texture, const float *xy, int xy_stride, const SDL_FColor *color, int color_stride, const float *uv, int uv_stride, int num_vertices, const void *indices, int num_indices, int size_indices)
    {
    ...
    SDL_INLINE bool SetTarget( SDLTexture texture )
    {
    ...
    SDL_INLINE SDLTexture GetRenderTarget( void ) const
    {
        return SDLTexture( SDL_GetRenderTarget(renderer ) );
    }
    
    SDL_INLINE SDLSurface RenderReadPixels( const SDL_Rect *rect ) const
    {
        return SDLSurface( SDL_RenderReadPixels( renderer, rect ) );
    }

    SDL_INLINE const SDLWindow  GetWindow( void ) const 
    {
        return SDLWindow( SDL_GetRenderWindow( renderer ) );
    }
...
public:
    SDLTexture( void );
    SDLTexture( const SDLTexture &ref ) : texture( ref.texture ) {}
    SDLTexture( SDL_Texture* _texture ) : texture( _texture ) {} 
    ~SDLTexture( void );

    SDL_INLINE bool CreateTexture( const SDLRenderer &renderer, const SDL_PixelFormat format, const SDL_TextureAccess access, const int w, const int h )
    {
        texture = SDL_CreateTexture( renderer, format, access, w, h );
        return texture != nullptr; 
    }

    SDL_INLINE bool CreateTextureFromSurface( const SDLRenderer &renderer, const SDLSurface &surface)
    {
        texture = SDL_CreateTextureFromSurface( renderer, surface );
        return texture != nullptr;
    }
    
    SDL_INLINE bool CreateTextureWithProperties( const SDLRenderer &renderer, SDL_PropertiesID props )
    {
    ...
    {
        return SDL_UpdateNVTexture( texture, rect, Yplane, Ypitch, UVpitch, UVpitch );
    }
    ...
    SDL_INLINE SDLRenderer GetRendererFromTexture( void ) const
    {
