...
*/
        ...
        public:
            ...
            Device( SDL_GPUDevice *device ) : device( device ) {}
            Device( const Device &device ) : device( device.device ) {}
            ~Device( void ) { Destroy(); }
...
*/
        ...
        {
        public:
            Fence( void ) : fence( nullptr ) {}
            Fence( SDL_GPUFence *fence ) : fence( fence ) {}
            Fence( const Fence &fence ) : fence( fence.fence ) {}
            ~Fence( void ){}

            SDL_INLINE bool Query( const Device &device )
            {
                return SDL_QueryGPUFence( device, fence );
            }

            SDL_INLINE void WaitForFence( const Device &device, bool wait_all ) const
            {
                SDL_WaitForGPUFences( device, wait_all, &fence,  1 );
            }

            SDL_INLINE void Release( const Device &device )
            {
                if( fence != nullptr )
                {
...
*/
        ...
        {
        public:
            Buffer( void ) : buffer( nullptr ) {}
            Buffer( SDL_GPUBuffer *buffer ) : buffer( buffer ) {}
            Buffer( const Buffer &buffer ) : buffer( buffer.buffer ) {}
            ~Buffer( void ) {}

            SDL_INLINE bool Create( const Device &device, const SDL_GPUBufferCreateInfo *createinfo )
            {
            ...
            SDL_INLINE void SetName( const Device &device, const char *name ) const
            {
...
*/
        ...
        {
        public:
            TransferBuffer( void ) : transferBuffer( nullptr ) {}
            TransferBuffer( SDL_GPUTransferBuffer *transferBuffer ) : transferBuffer( transferBuffer ) {}
            TransferBuffer( const TransferBuffer &transferBuffer ) : transferBuffer( transferBuffer.transferBuffer ) {}
            ~TransferBuffer( void ) {}

            SDL_INLINE bool Create( const Device &device, const SDL_GPUTransferBufferCreateInfo *createinfo )
            {
            ...
            SDL_INLINE void* Map( const Device &device, const bool cycle ) const
            {
                return SDL_MapGPUTransferBuffer( device, transferBuffer, cycle );
            }

            SDL_INLINE void Unmap( const Device &device )
            {
...
*/
        ...
        {
        public:
            Shader( void ) : shader( nullptr ) {}
            Shader( SDL_GPUShader *shader ) : shader( shader ) {}
            Shader( const Shader &shader ) : shader( shader.shader ) {}
            ~Shader( void ) {}

            SDL_INLINE bool Create( const Device &device, const SDL_GPUShaderCreateInfo *createinfo )
            {
...
*/
        ...
        {
        public:
            Sampler( void ) : sampler( nullptr ){}
            Sampler( SDL_GPUSampler *sampler ) : sampler( sampler ) {}
            Sampler( const Sampler &sampler ) : sampler( sampler.sampler ) {}
            ~Sampler( void ){}

            SDL_INLINE bool Create( const Device &device , const SDL_GPUSamplerCreateInfo *createinfo )
            {
...
*/
        ...
        {
        public:
            CommandBuffer( void ) : commandBuffer( nullptr ) {};
            CommandBuffer( SDL_GPUCommandBuffer *commandBuffer ) : commandBuffer( commandBuffer ) {}
            CommandBuffer( const CommandBuffer &commandBuffer ) : commandBuffer( commandBuffer.commandBuffer ) {}
            ~CommandBuffer( void ){}

            SDL_INLINE bool Acquire( const Device &device )
            {
...
*/
        ...
        {
        public:
            GraphicsPipeline( void ) : graphicsPipeline( nullptr ) {}
            GraphicsPipeline( SDL_GPUGraphicsPipeline *pipeline ) : graphicsPipeline( pipeline ) {}
            GraphicsPipeline( const GraphicsPipeline &pipeline ) : graphicsPipeline( pipeline.graphicsPipeline ) {}
            ~GraphicsPipeline( void ) {}

            SDL_INLINE bool Create( const Device &device, const SDL_GPUGraphicsPipelineCreateInfo *createinfo )
            {
...
*/
        ...
        {
        public:
            RenderPass( void ) : renderPass( nullptr ) {}
            RenderPass( SDL_GPURenderPass *renderPass ) : renderPass( renderPass ) {}
            RenderPass( const RenderPass &renderPass ) : renderPass( renderPass.renderPass ) {}
            ~RenderPass( void ) {}

            SDL_INLINE bool Begin( CommandBuffer command_buffer, const SDL_GPUColorTargetInfo *color_target_infos, Uint32 num_color_targets, const SDL_GPUDepthStencilTargetInfo *depth_stencil_target_info )
            {
            ...
            SDL_INLINE void BindGraphicsPipeline( const GraphicsPipeline &pipeline ) const
            {
            ...
            SDL_INLINE void DrawGPUPrimitivesIndirect( Buffer buffer, Uint32 offset, Uint32 draw_count ) const
            {
...
*/
    ...
    public: 
        CopyPass( void );
        CopyPass( SDL_GPUCopyPass *copyPass ) : copyPass( copyPass ) {};
        CopyPass( CopyPass &copyPass) : copyPass( copyPass.copyPass ) {}
        ~CopyPass( void ){};

        SDL_INLINE bool Begin( CommandBuffer command_buffer )
        {
...
*/
        ...
        {
        public:
            Texture( void ) : texture( nullptr ) {}
            Texture( const Texture &ref ) : texture( ref.texture ) {}
            Texture( SDL_GPUTexture *texture ) : texture( texture ) {};
            ~Texture( void ) {}

            SDL_INLINE bool Create( const Device &device, const SDL_GPUTextureCreateInfo *createinfo )
            {
            ...
            SDL_INLINE void SetTextureName( const Device &device, const char *name ) const
            {
                SDL_SetGPUTextureName( device, texture, name );
            }

            SDL_INLINE void GenerateMipmaps( CommandBuffer commandBuffer ) const
            {
...
*/
        ...
        {
        public:
            ComputePass( void ) : computePass( nullptr ) {}
            ComputePass( SDL_GPUComputePass* computepass ) : computePass( computepass ) {}
...
*/
        ...
        {
        public:
            ComputePipeline( void ) : computePipeline( nullptr ) {}
            ComputePipeline( SDL_GPUComputePipeline *pipeline ) : computePipeline( pipeline ) {}
            ComputePipeline( const ComputePipeline &ref ) : computePipeline( ref.computePipeline ) {}
            ~ComputePipeline( void ) {}
            
            SDL_INLINE bool Create( const Device &device, const SDL_GPUComputePipelineCreateInfo *createinfo )
            {
                computePipeline = SDL_CreateGPUComputePipeline( device, createinfo );
                return computePipeline != nullptr;
            }

            SDL_INLINE void Bind( ComputePass compute_pass ) const
            {
